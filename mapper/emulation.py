...
class EmulatedWorld(World):
	...
	def __init__(self, interface, findFormat):
		...
		self.output(f"Loaded {len(self.rooms)} rooms.")
	...
	def user_command_partial_look(self, *args):
		...
		for direction, exitObj in self.sortExits(self.currentRoom.exits):
			...
			if exitObj.door or "door" in exitObj.exitFlags:
				doorList.append(f"{direction}: {exitObj.door if exitObj.door else 'exit'}")
		...
		self.output(f"Exits: {', '.join(exitList)}")
		if self.currentRoom.note:
			self.output(f"Note: {self.currentRoom.note}")
		...
		if self.config.get("show_vnum", True):
			self.output(f"Vnum: {self.currentRoom.vnum}")
	...
	def user_command_partial_exits(self, *args):
		...
		for direction, exitObj in self.sortExits(self.currentRoom.exits):
			exitLine = []
			exitLine.append(f"{direction.capitalize()}:")
			if exitObj.door or "door" in exitObj.exitFlags:
				exitLine.append(
					f"{'hidden' if 'hidden' in exitObj.doorFlags else 'visible'} "
					+ f"({exitObj.door if exitObj.door else 'exit'}),"
				)
			if exitObj.to.isdigit() and exitObj.to in self.rooms:
				exitLine.append(f"{self.rooms[exitObj.to].name}, {self.rooms[exitObj.to].terrain}")
	...
	def user_command_brief(self, *args):
		...
		self.output(f"Brief mode {'enabled' if status else 'disabled'}.")
	...
	def user_command_terrain(self, *args):
		...
		self.output(f"Terrain symbols in prompt {'enabled' if status else 'disabled'}.")

	def user_command_vnum(self, *args):
		status = self.toggleSetting("show_vnum")
		self.output(f"Show room vnum {'enabled' if status else 'disabled'}.")
	...
	def move(self, text):
		...
		if text in DIRECTIONS:
			...
			return self.output(f"Error: {text} isn't a direction, label, or vnum.")
		if vnum == "undefined":
			...
			return self.output(f"Error: no rooms in the database with vnum ({vnum}).")
			...
			if os.path.exists(fileName):
				if not os.path.isdir(fileName):
					try:
						...
						self.output(f"{e.strerror}: '{e.filename}'")
						return {}
					except ValueError:
						self.output(f"Corrupted configuration file: {fileName}")
						return {}
				else:
					self.output(f"Error: '{fileName}' is a directory, not a file.")
					return {}
			else:
				return {}
	...
	def parseInput(self, userInput):
		...
		if direction:
			...
			getattr(self, f"user_command_partial_{completed}")(arguments)
		elif command in userCommands:
			getattr(self, f"user_command_{command}")(arguments)
